...
Im Terminal lassen sich diese mit folgendem Befehl installieren:
```
xcode-select --install
```

Nun lässt sich auch Homebrew wie folgt herunterladen und einrichten:
```
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
```

Das Skript erklärt den Installationsprozess und weist auf Nutzereingaben hin.

Am Ende der installation werden `Next steps:` angezeigt.

Diese sind wichtig um Homebrew dem PATH hinzuzufügen und von überall aus dem Terminal Zugriff zu haben.

Die angezeigten Befehlen nach den `Next steps:` können dafür einfach so in das Terminal kopiert und ausgeführt werden. 

#### Verifizierung der Homebrew Installation


Mit folgendem Befehl lässt sich die Installation auf Fehler überprüfen:
```
brew doctor
```

Sollte das Terminal nicht `Your system is ready to brew` ausgeben, sondern eine Problemmeldung, gibt Homebrew Anweisungen das Problem zu lösen.

Vor der Nutzung sollte auch immer auf Updates überprüft werden mit:
```
brew update
```
...
Durch Homebrew lässt sich Python leicht im Terminal installieren:
```
brew install python
```
...
Nun können wir mit Homebrew das [Whisper Paket](https://github.com/openai/whisper/tree/main?tab=readme-ov-file) installieren. Wir installieren Whisper global, was heißt, dass es erst einmal egal ist, in welchem Ordner wir uns im Terminal befinden.
...
Zum Beispiel, aktuell befinde ich mich im Terminal in `/Users/legie` (mit `pwd` angezeigt). Nun kann man mit folgendem Befehl in den gewünschten Projektordner wechseln:
...
Als ersten Test verwenden wir whisper mit den gegebenen Voreinstellungen. Dafür den Befehl `whisper` mit dem Namen der Audiodatei als Argument ausführen, z.B.:
